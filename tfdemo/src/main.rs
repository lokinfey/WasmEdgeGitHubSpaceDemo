...
fn infer_image() {
    ...
    let tensor_data = image_to_tensor(image_name.to_string(), 300, 300);
    ...
    for i in 0..3 {
        println!(
            ...
            results[i].0,
            results[i].1,
            startwar_classes::STAR_WAR[results[i].0]
        );
    }
}
...
fn image_to_tensor1(path: String, height: u32, width: u32) -> Vec<u8> {
